...
{
    public JSONTable(@NotNull final String userNameSpace, @NotNull final String name) {
    ...
    public void delete(@NotNull final Function1<? super JsonObject, Boolean> checker) {
        ...
        if (removeIndexList.size() > 0) {
            ...
            int l = 0;
            final int size2 = dataList.size();
            if (l < size2) {
                do {
                    final int j = l;
                    ++l;
                    if (!removeIndexList.contains(j)) {
                        newList.add(dataList.getJsonObject(j));
                    }
                } while (l < size2);
            }
            dataList = newList;
