...
{
    @NotNull
    public JsonElement serialize(@Nullable final Long src, @Nullable final Type typeOfSrc, @Nullable final JsonSerializationContext context) {
        return (JsonElement)new JsonPrimitive(String.valueOf(src));
    }
    
    @Nullable
    public Long deserialize(@NotNull final JsonElement json, @Nullable final Type typeOfT, @Nullable final JsonDeserializationContext context) {
        Intrinsics.checkNotNullParameter((Object)json, "json");
        Long n;
        if (json.isJsonPrimitive()) {
            final JsonPrimitive prim = json.getAsJsonPrimitive();
            n = (prim.isNumber() ? Long.valueOf(prim.getAsNumber().longValue()) : null);
        }
        else {
            n = null;
        }
