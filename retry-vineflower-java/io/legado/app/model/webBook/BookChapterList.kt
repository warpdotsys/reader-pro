...
public object BookChapterList {
   public suspend fun analyzeChapterList(book: Book, body: String?, bookSource: BookSource, baseUrl: String, redirectUrl: String, debugLog: DebugLog? = ...): List<
      ...
      label170: {
                                                ...
                                                switch (this.label) {
                                                   case 0:
                                                      ...
                                                      val analyzeUrl: AnalyzeUrl = new AnalyzeUrl(
                                                         ...
                                                         this.$book,
                                                         null,
                                                         io.legado.app.data.entities.BaseSource.DefaultImpls.getHeaderMap$default(
                                                            this.$bookSource, false, 1, null
                                                         ),
                                                         this.$debugLog,
                                                         318,
                                                         null
         ...
         while (true) {
            ...
            var var65: AnalyzeUrl = new AnalyzeUrl(
               ...
               book,
               null,
               io.legado.app.data.entities.BaseSource.DefaultImpls.getHeaderMap$default(bookSource, false, 1, null),
               debugLog,
               318,
               null
            );
      ...
      if (chapterList.isEmpty()) {
         ...
         if (debugLog != null) {
            DefaultImpls.log$default(debugLog, book.getOrigin(), Intrinsics.stringPlus("◇目录总数:", Boxing.boxInt(var41.size())), false, 4, null);
         }
   ...
   private suspend fun analyzeChapterList(
      ...
      if (log && debugLog != null) {
         DefaultImpls.log$default(debugLog, bookSource.getBookSourceUrl(), "┌获取目录列表", false, 4, null);
      }

      val var36: java.util.List = analyzeRule.getElements(listRule);
      if (log && debugLog != null) {
         DefaultImpls.log$default(debugLog, bookSource.getBookSourceUrl(), Intrinsics.stringPlus("└列表大小:", Boxing.boxInt(var36.size())), false, 4, null);
      }
      ...
      if (getNextUrl && var37 != null && var37.length() != 0) {
         if (log && debugLog != null) {
            DefaultImpls.log$default(debugLog, bookSource.getBookSourceUrl(), "┌获取目录下一页列表", false, 4, null);
         }
         ...
         if (log && debugLog != null) {
            DefaultImpls.log$default(debugLog, bookSource.getBookSourceUrl(), Intrinsics.stringPlus("└", TextUtils.join("，\n", nextUrlList)), false, 4, null);
         }
      }
      ...
      if (!var36.isEmpty()) {
         if (log && debugLog != null) {
            DefaultImpls.log$default(debugLog, bookSource.getBookSourceUrl(), "┌解析目录列表", false, 4, null);
         }
         ...
         for (Object item$iv : $this$forEachIndexed$iv) {
            ...
            if (bookChapter.getUrl().length() == 0) {
               if (bookChapter.isVolume()) {
                  ...
                  if (log && debugLog != null) {
                     DefaultImpls.log$default(debugLog, bookSource.getBookSourceUrl(), "⇒一级目录$index未获取到url,使用标题替代", false, 4, null);
                  }
               } else {
                  bookChapter.setUrl(baseUrl);
                  if (log && debugLog != null) {
                     DefaultImpls.log$default(debugLog, bookSource.getBookSourceUrl(), "⇒目录$index未获取到url,使用baseUrl替代", false, 4, null);
                  }
               }
            }
            ...
            if (bookChapter.getTitle().length() > 0) {
               ...
               DefaultImpls.log$default(debugLog, bookSource.getBookSourceUrl(), "章节名为空", false, 4, null);
            }
         }

         if (log && debugLog != null) {
            DefaultImpls.log$default(debugLog, bookSource.getBookSourceUrl(), "└目录列表解析完成", false, 4, null);
         }

         if (chapterList.size() > 0) {
            if (log && debugLog != null) {
               DefaultImpls.log$default(debugLog, bookSource.getBookSourceUrl(), "≡首章信息", false, 4, null);
            }
            ...
            DefaultImpls.log$default(debugLog, bookSource.getBookSourceUrl(), "章节列表为空", false, 4, null);
         }
      }
