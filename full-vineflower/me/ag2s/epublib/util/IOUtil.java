...
public class IOUtil {
   ...
   public static long skip(final InputStream input, final long toSkip) throws IOException {
      if (toSkip < 0L) {
         ...
         while (remain > 0L) {
            long n = input.read(SKIP_BYTE_BUFFER, 0, (int)Math.min(remain, (long)SKIP_BYTE_BUFFER.length));
            if (n < 0L) {
               break;
            }
   ...
   public static long skip(final Reader input, final long toSkip) throws IOException {
      if (toSkip < 0L) {
         ...
         while (remain > 0L) {
            long n = input.read(SKIP_CHAR_BUFFER, 0, (int)Math.min(remain, (long)SKIP_BYTE_BUFFER.length));
            if (n < 0L) {
               break;
            }
