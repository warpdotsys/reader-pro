...
1. `clean test` 有 61/61 通过，但 `bootJar` 仍无法确定主类并直接失败。
2. 目标应用与 Legado 范围仍缺 13 个顶层符号，集中在入口、配置、剩余业务 controller 和远程 WebView 运行栈。
3. 本批恢复 `LicenseController` 及其 Kotlin facade；其公开 descriptor 已与原 JAR 零差异对账。
...
| 口径 | 原 JAR | 当前编译产物 | 差异 |
| --- | ---: | ---: | ---: |
| 全部顶层 class | 276 | 282 | 缺 13，多 19 |
| 项目与 Legado 顶层符号 | 198 | 185 | 缺 13 |
...
按目标 class 的 `SourceFile` 聚合后，项目与 Legado 范围共有 163 个源码单元。当前功能上覆盖 155 个，仍有 8 个 Kotlin 源码单元未恢复。已映射的 263 个共同顶层类型语言归属全部一致：160 Kotlin、103 Java，未发现 Java/Kotlin 边界错配。
...
当前编译产物仍未闭合这 13 个入口与 API 符号，说明工程尚未恢复完整 HTTP 运行闭包。
...
| `LicenseController` | 许可证读取、导入、签发、解密、激活、检查与邮件试用控制器及 Kotlin facade 缺失 | 已恢复；目标公开 descriptor 零差异对账，覆盖本地许可证、RSA 密钥、验证码缓存、试用授权与许可证有效性路径；远端激活协议仍待后续行为回炉 |
...
- 当前有 17 个测试类、61 个测试，`clean test` 为 61/61 通过。
