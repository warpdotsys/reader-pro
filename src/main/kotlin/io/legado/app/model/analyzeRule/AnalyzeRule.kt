 ...
 * 解析规则获取结果
 */
@Suppress("unused", "RegExpRedundantEscape", "UNNECESSARY_SAFE_CALL")
class AnalyzeRule(
    ...
    fun reGetBook() {
        ...
        if (bookSource != null && book != null) {
            runBlocking {
                withTimeout(1_800_000L) {
                    WebBook(bookSource, userNameSpace = getUserNameSpace())
                        .preciseSearch(book.name, book.author)
                        .getOrThrow()
                        .let {
                            book.bookUrl = it.bookUrl
                            it.variableMap.forEach { (key, value) ->
                                book.putVariable(key, value)
                            }
                            WebBook(bookSource, userNameSpace = getUserNameSpace())
    ...
    fun refreshBookUrl() {
        runBlocking {
            ...
            val books = WebBook(bookSource, userNameSpace = getUserNameSpace())
