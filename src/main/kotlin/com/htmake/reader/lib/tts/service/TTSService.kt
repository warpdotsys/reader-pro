...
class TTSService private constructor(private var outputFormat:OutputFormat?, private val usingAzureApi:Boolean) {
    ...
    private val listener=object:WebSocketListener(){ override fun onClosed(webSocket:WebSocket,code:Int,reason:String){ws=null;synthesising=false}; override fun onClosing(webSocket:WebSocket,code:Int,reason:String){ws=null;synthesising=false}; override fun onFailure(webSocket:WebSocket,t:Throwable,response:Response?){ws=null;synthesising=false}; override fun onMessage(webSocket:WebSocket,text:String){if(text.contains(TtsConstants.TURN_START))audioBuffer.clear() else if(text.contains(TtsConstants.TURN_END)){latch!!.countDown();synthesising=false}}; override fun onMessage(webSocket:WebSocket,bytes:ByteString){val audioIndex=bytes.lastIndexOf(TtsConstants.AUDIO_START.encodeToByteArray())+TtsConstants.AUDIO_START.length; val audioType=bytes.lastIndexOf(TtsConstants.AUDIO_CONTENT_TYPE.encodeToByteArray())+TtsConstants.AUDIO_CONTENT_TYPE.length!=-1; if(audioIndex!=-1&&audioType)try{audioBuffer.write(bytes.substring(audioIndex.toLong()))}catch(_:Exception){}} }
